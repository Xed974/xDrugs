            ...
            while open do
                ...
                RageUI.IsVisible(mainMenu, function()
                    if recolte == true then
                        RageUI.ButtonWithStyle('Récolter de la weed', nil, {RightBadge = RageUI.BadgeStyle.Weed}, true, function(Hovered, s, Selected)                                                                                                                   
                            if Selected then
                                exports['progressBars']:startUI(5000, "Récolte en cours...")
                                TaskStartScenarioInPlace(PlayerPedId(), 'WORLD_HUMAN_GARDENER_PLANT', 0, true)
                                Wait(5000)
                                TriggerServerEvent('xDrugs:recolteweed')
                                recolte = false
                                Wait(1000)
                                recolte = true
...
Citizen.CreateThread(function()
    while true do
        local wait = 750
        for k in pairs(Config.Position.Weed.Recolte) do
        local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
        local pos = Config.Position.Weed.Recolte
