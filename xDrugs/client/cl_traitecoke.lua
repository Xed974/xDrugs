...
RegisterNetEvent('xDrugs:cokeOn')
AddEventHandler('xDrugs:cokeOn',function()
    ...
    Wait(1000)
    TriggerServerEvent('xDrugs:traitercoke')
            ...
            while open do
                ...
                RageUI.IsVisible(mainMenu, function()
                    if traiter == true then
                        RageUI.ButtonWithStyle('Traiter de la coca', nil, {RightBadge = RageUI.BadgeStyle.Coke}, true, function(Hovered, s, Selected)                                                                                                                   
                            if Selected then
                                local type = 2
...
Citizen.CreateThread(function()
    while true do
        local wait = 750
        for k in pairs(Config.Position.Coke.Traitement) do
        local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
        local pos = Config.Position.Coke.Traitement
